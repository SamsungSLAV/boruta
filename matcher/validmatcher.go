...
func (m ValidMatcher) Notify(r []boruta.ReqID) {
	logger.WithProperty("type", "Validmatcher").WithProperty("method", "Notify").
		Debugf("ValidMatcher notified about following requests: %v", r)
...
func (m ValidMatcher) iterateRequests() bool {
	...
	if err != nil {
		...
		logger.WithProperty("type", "Validmatcher").WithProperty("method", "iterateRequests").
			Critical("Critical logic error. No iterations over requests collection should be running.")
...
func (m ValidMatcher) matchWorkers(req boruta.ReqInfo) bool {
	logger.WithProperty("type", "Validmatcher").WithProperty("method", "matchWorkers").
		WithProperty("requestID", req.ID.String()).
		Debug("Start.")
	...
	// Match found.
	logger.WithProperty("type", "Validmatcher").WithProperty("method", "matchWorkers").
		WithProperty("requestID", req.ID.String()).WithProperty("workerID", worker).
