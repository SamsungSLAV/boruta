...
// filter by request IDs, states and priorities.
// All enabled filter fields (not nil nor empty) must be satisfied to make a match (AND behaviour).
// In a scope of a single filter field "one of" matches are used (OR behaviour). Empty or nil slice
// ignores filter for that type.
